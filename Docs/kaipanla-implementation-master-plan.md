...
## 8. 长期演进目标：在 SKILL 引导下共创并沉淀新判定器

除完成“单次自然语言任务闭环”外，系统还应具备一个更长期、但同样重要的演进能力：

**人类与 OpenClaw 能在 SKILL 的探索模式引导下，通过持续性的多轮执行、证据回读、ask-human 与人工确认，逐步形成新的任务判定经验，并把成熟经验沉淀为代码级判定器。**

这不是让 SKILL 永久替代代码判定，也不是让代码一次性写死所有意图，而是形成下面的闭环：

1. 先由代码提供一批初始化判定器，覆盖高频、低歧义、已验证的任务模式
2. 对于新意图、弱意图、模糊目标或当前判定器不足的任务，AI 在 SKILL 指导下进入 exploration 模式
3. AI 基于事实证据与人类反馈，逐步识别：
   - 哪些 UI / request / structure 信号真正代表目标完成
   - 哪些只是噪声或伪增信
   - 哪些动作路径可复现，哪些只在偶然条件下成立
4. 当某类意图经过足够轮次验证、边界清晰、结果可解释且经人类确认后，再把这类经验下沉为代码级判定器或稳定 recipe

一句话：

**SKILL 负责帮助系统探索未知意图，代码负责固化已验证意图。**

### 8.1 这个长期目标的验收口径

最终应能做到：
- 人类提出一个当前系统尚不稳定支持的新意图
- OpenClaw 在 SKILL 引导下进行多轮探索与对话
- 过程中能明确区分“已知能力”“未知能力”“需人工确认部分”
- 经过若干次成功探索后，形成新的代码级判定器/recipe，并被后续任务复用

### 8.2 该能力属于哪个阶段

这项能力通常跨越 Round2 与 Round3：
- Round2 提供探索闭环与 ask-human 能力
- Round3 提供从探索经验沉淀到稳定交付能力的 promote/固化机制

因此，后续任何实现或评估都应把“新判定器的生成与沉淀能力”视为最终架构的一部分，而不是额外可选项。
